...
function normalizeCategory(value: unknown): ProductCategory | null {
  ...
  return { id, name, nameDe, visible: raw.visible !== false };
}

export async function GET() {
  const categories = await getProductCategories();
  return NextResponse.json(
    { categories, configured: isCategoryStorageConfigured() },
...
export async function PUT(request: Request) {
  ...
  if (!isCategoryStorageConfigured()) {
    return NextResponse.json({ error: "Vercel Blob is not configured yet." }, { status: 503 });
  }

  const body = (await request.json().catch(() => null)) as { categories?: unknown } | null;
  if (!Array.isArray(body?.categories) || body.categories.length === 0 || body.categories.length > 30) {
  ...
  const usedIds = products.map(
    (product) => product.categoryId || categoryIdFromName(product.category) || "jewelry",
  );
  const missing = usedIds.find((id) => !categoryMap.has(id));
  if (missing) {
    return NextResponse.json(
      { error: "Diese Kategorie wird noch von Produkten verwendet. Weise die Produkte zuerst einer anderen Kategorie zu." },
      { status: 400 },
    );
  }
