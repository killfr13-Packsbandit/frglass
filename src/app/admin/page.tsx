...
export default function Page() {
  ...
  useEffect(() => {
    ...
    readSession().then((value) => { if (active) setSession(value); }).catch(() => {
      if (active) {
        setSession({ authenticated: false, authConfigured: false });
        setError("Anmeldung konnte nicht geprüft werden. Bitte lade die Seite erneut.");
      }
    });
    return () => { active = false; };
  }, []);
  ...
  async function login(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        setError(response.status === 401 ? "Passwort stimmt nicht." : "Anmeldung ist gerade nicht verfügbar. Bitte versuche es erneut.");
        return;
      }
      const nextSession = await refreshSession();
      if (!nextSession.authenticated) {
        setError("Das Passwort wurde akzeptiert, aber die Anmeldung konnte nicht gespeichert werden. Bitte öffne die Seite in einem normalen Tab und erlaube Cookies für frglass.at.");
        return;
      }
      setPassword("");
    } catch {
  ...
  return <main className="min-h-screen bg-black px-4 py-24 text-white sm:px-8 sm:py-28"><section className="mx-auto max-w-5xl">
    ...
    {session?.authenticated && <div className="mt-10"><p className="max-w-2xl leading-7 text-neutral-400">Die Bereiche, die du für die Website wirklich brauchst.</p><div className="mt-8 grid gap-5 sm:grid-cols-2">
      ...
      <Card href="/admin/pages" icon="✎" title="Website-Inhalte" text="Texte, Seitenbilder und zusätzliche frei platzierbare Inhalte aus Text, Bild oder Video." />
