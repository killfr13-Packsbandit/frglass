...
const pages: PageDefinition[] = [
  {
    id: "home",
    label: "Startseite",
    description: "Die sichtbaren Inhalte deiner Startseite, nach Bereichen sortiert.",
    fields: [
      ...
      ...localized("home.jewelry.eyebrow", "Kleine Überschrift", "Anhänger"),
      ...localized("home.jewelry.title", "Überschrift", "Anhänger"),
      ...localized("home.jewelry.intro", "Kurztext", "Anhänger", "textarea"),

      ...localized("home.workshop.eyebrow", "Kleine Überschrift", "Werkstatt"),
      ...localized("home.workshop.title", "Überschrift", "Werkstatt"),
      ...localized("home.workshop.text", "Text", "Werkstatt", "textarea"),
      ...
      ...localized("home.studio.eyebrow", "Kleine Überschrift", "Studio-Teaser"),
      ...localized("home.studio.title", "Überschrift", "Studio-Teaser"),
      ...localized("home.studio.text", "Text", "Studio-Teaser", "textarea"),
      ...localized("home.studio.button", "Button", "Studio-Teaser"),
      media("home.studio.media", "Bild / Video", "Studio-Teaser"),
    ],
  },
  {
    id: "about",
  ...
  },
  {
    id: "studio",
    label: "Studio",
    description: "Texte der Studio-Seite. Die freie Foto-/Video-Galerie liegt im separaten Medieneditor.",
    mediaManager: "studio",
    fields: [
      ...
      ...localized("studio.card1.title", "Karte 1 Titel", "Angebote"),
      ...localized("studio.card1.text", "Karte 1 Text", "Angebote", "textarea"),
      ...localized("studio.card2.title", "Karte 2 Titel", "Angebote"),
      ...localized("studio.card2.text", "Karte 2 Text", "Angebote", "textarea"),
      ...localized("studio.card3.title", "Karte 3 Titel", "Angebote"),
      ...localized("studio.card3.text", "Karte 3 Text", "Angebote", "textarea"),
      ...localized("studio.card4.title", "Karte 4 Titel", "Angebote"),
      ...localized("studio.card4.text", "Karte 4 Text", "Angebote", "textarea"),
  ...
  },
  {
    id: "shop",
    label: "Shop",
    description: "Nur die allgemeine Shop-Überschrift und Einleitung. Anhänger bearbeitest du unter Produkte.",
    fields: [
  ...
  },
  {
    id: "gallery",
    label: "Galerie",
    description: "Nur Überschrift und Einleitung. Bilder und Videos liegen im separaten Galerie-Editor.",
...
export default function Page() {
  ...
  const visibleFields = useMemo(
    () => definition.fields.filter((field) => field.kind === "media" || field.language === editorLanguage),
    [definition, editorLanguage],
  );
  ...
  async function uploadMedia(event: ChangeEvent<HTMLInputElement>, field: MediaField) {
    ...
    try {
      ...
      const blob = await upload(`site/media/${Date.now()}-${safeName}`, prepared, {
        access: "public",
        handleUploadUrl: "/api/site-content/upload",
      });
      update(field.key, blob.url);
      update(field.typeKey, prepared.type.startsWith("video/") ? "video" : "image");
    } catch {
    ...
    return (
      <main className="min-h-screen bg-black px-5 py-28 text-white">
        <section className="mx-auto max-w-4xl">
          <h1 className="text-4xl font-black uppercase">Website-Inhalte</h1>
      ...
      <form onSubmit={save} className="mx-auto max-w-6xl">
        ...
        <div className="mt-3 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-4 max-w-3xl leading-7 text-neutral-400">
              Wähle zuerst die Seite und dann Deutsch oder Englisch. Du siehst nur die Felder, die wirklich zu diesem Bereich gehören.
            </p>
          </div>
          <button type="submit" disabled={saving || Boolean(uploadingKey)} className="rounded-full bg-white px-6 py-3 text-sm font-black uppercase tracking-wider text-black disabled:opacity-50">
        ...
        <div className="mt-8 grid gap-4 rounded-3xl border border-white/10 bg-white/[0.025] p-4 sm:p-5 lg:grid-cols-[1fr_auto] lg:items-center">
          <div className="flex gap-2 overflow-x-auto pb-1">
            {pages.map((page) => (
              <button key={page.id} type="button" onClick={() => setActivePage(page.id)} className={`whitespace-nowrap rounded-full border px-4 py-2 text-xs font-bold uppercase tracking-wider transition ${activePage === page.id ? "border-orange-300 bg-orange-300 text-black" : "border-white/15 text-neutral-300"}`}>
                {page.label}
              </button>
            ))}
          </div>
          ...
          <div className="flex w-fit items-center rounded-full border border-white/15 bg-black/40 p-1 text-xs font-bold">
            <button type="button" onClick={() => setEditorLanguage("de")} className={`rounded-full px-4 py-2 transition ${editorLanguage === "de" ? "bg-white text-black" : "text-neutral-400"}`}>DE</button>
            <button type="button" onClick={() => setEditorLanguage("en")} className={`rounded-full px-4 py-2 transition ${editorLanguage === "en" ? "bg-white text-black" : "text-neutral-400"}`}>EN</button>
          </div>
        </div>
                ...
                <div className="mt-5 grid gap-5 sm:grid-cols-2">
                  {visibleFields.filter((field) => field.section === section).map((field) => {
                    if (field.kind === "media") {
                      ...
                      return (
                        <div key={field.key} className="rounded-2xl border border-white/10 bg-black/30 p-4 sm:col-span-2">
                          <div className="flex flex-wrap items-center justify-between gap-3">
                            <div>
                              <p className="font-bold">{field.label}</p>
                              <p className="mt-1 text-xs text-neutral-500">Bild oder Video, max. 100 MB.</p>
                            </div>
                            <label className="cursor-pointer rounded-full border border-orange-300 px-4 py-2 text-xs font-bold uppercase tracking-wider text-orange-300">
                              ...
                              <input type="file" accept="image/jpeg,image/png,image/webp,image/gif,video/mp4,video/quicktime,video/webm" onChange={(event) => uploadMedia(event, field)} disabled={Boolean(uploadingKey)} className="hidden" />
                            </label>
                          </div>
                          {url ? (
                            <div className="mt-4">
                              <div className="overflow-hidden rounded-xl bg-neutral-950">
                                {mediaType === "video" ? <video src={url} controls playsInline className="max-h-[420px] w-full object-contain" /> : <img src={url} alt="" className="max-h-[420px] w-full object-contain" />}
                              </div>
                              <button type="button" onClick={() => { update(field.key, ""); update(field.typeKey, "image"); }} className="mt-3 text-xs font-bold uppercase tracking-wider text-red-300">Medium entfernen</button>
                            </div>
                    ...
                    return (
                      <label key={field.key} className={`grid gap-2 ${field.kind === "textarea" ? "sm:col-span-2" : ""}`}>
                        <span className="text-sm font-bold">{field.label}</span>
                        {field.kind === "textarea" ? (
                          <textarea value={content[field.key] ?? ""} onChange={(event) => update(field.key, event.target.value)} rows={5} className={`${common} resize-y`} />
                        ) : (
                          <input value={content[field.key] ?? ""} onChange={(event) => update(field.key, event.target.value)} className={common} />
                        )}
                      </label>
                    );
                  })}
        ...
        <div className="sticky bottom-4 mt-8 flex justify-end">
          <button type="submit" disabled={saving || Boolean(uploadingKey)} className="rounded-full bg-orange-300 px-6 py-4 text-sm font-black uppercase tracking-wider text-black shadow-2xl shadow-black disabled:opacity-50">
