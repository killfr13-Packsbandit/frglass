...
function aspectFor(key: string) {
  ...
  if (key.includes("workshop.main")) return "aspect-video";
  if (key.includes("workshop.media")) return "aspect-[4/3]";
...
export default function Page() {
  ...
  return <main className="min-h-screen bg-black px-4 py-24 text-white sm:px-6 sm:py-28"><form onSubmit={save} className="mx-auto max-w-6xl">
    ...
    <div className="mt-3 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><h1 className="text-4xl font-black uppercase sm:text-6xl">Website-Inhalte</h1><p className="mt-4 max-w-3xl leading-7 text-neutral-400">Alle Seitenbilder kannst du hier direkt im echten Ausschnitt verschieben und zoomen. Das Fadenkreuz funktioniert mit Finger oder Maus.</p></div><button type="submit" disabled={saving || Boolean(uploadingKey)} className="rounded-full bg-white px-6 py-3 text-sm font-black uppercase tracking-wider text-black disabled:opacity-50">{saving ? "Speichert …" : "Speichern"}</button></div>
    ...
    <section className="mt-6 rounded-3xl border border-white/10 bg-white/[0.03] p-5 sm:p-8"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><h2 className="text-2xl font-black uppercase sm:text-3xl">{definition.label}</h2><p className="mt-3 max-w-3xl text-sm leading-6 text-neutral-400">{definition.description}</p></div>{managerHref && <Link href={managerHref} className="inline-flex shrink-0 rounded-full border border-orange-300 px-4 py-2 text-xs font-bold uppercase tracking-wider text-orange-300">{managerLabel}</Link>}</div>
      <div className="mt-8 space-y-6">{sections.map((section) => <div key={section} className="rounded-2xl border border-white/10 bg-black/25 p-4 sm:p-6"><h3 className="text-sm font-black uppercase tracking-[0.22em] text-orange-300">{section}</h3><div className="mt-5 grid gap-5 sm:grid-cols-2">{visibleFields.filter((field) => field.section === section).map((field) => {
        if (field.kind === "media") {
          ...
          return <div key={field.key} className="rounded-2xl border border-white/10 bg-black/30 p-4 sm:col-span-2"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-bold">{field.label}</p><p className="mt-1 text-xs text-neutral-500">Bild antippen/ziehen für den Ausschnitt. Max. 100 MB.</p></div><label className="cursor-pointer rounded-full border border-orange-300 px-4 py-2 text-xs font-bold uppercase tracking-wider text-orange-300">{uploadingKey === field.key ? "Upload …" : url ? "Ersetzen" : "Hochladen"}<input type="file" accept="image/jpeg,image/png,image/webp,image/gif,video/mp4,video/quicktime,video/webm" onChange={(event) => uploadMedia(event, field)} disabled={Boolean(uploadingKey)} className="hidden" /></label></div>
            {url ? <div className="mt-4">{mediaType === "video" ? <div className="overflow-hidden rounded-xl bg-neutral-950"><video src={url} controls playsInline className="max-h-[420px] w-full object-contain" /></div> : <ImageFocusEditor src={url} zoom={zoom} focusX={focusX} focusY={focusY} aspectClass={aspectFor(field.key)} onChange={(next) => setContent((current) => ({ ...current, ...(next.zoom !== undefined ? { [keys.zoom]: String(next.zoom) } : {}), ...(next.focusX !== undefined ? { [keys.x]: String(next.focusX) } : {}), ...(next.focusY !== undefined ? { [keys.y]: String(next.focusY) } : {}) }))} />}
