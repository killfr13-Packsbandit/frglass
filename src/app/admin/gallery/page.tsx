...
export default function Page() {
  ...
  async function uploadMedia(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      for (let index = 0; index < files.length; index += 1) {
        ...
        added.push({ id: crypto.randomUUID(), mediaUrl: blob.url, mediaType: isVideo ? "video" : "image", contentType: file.type, description: "", descriptionEn: "", createdAt: new Date().toISOString(), fit: "contain", position: "center", zoom: 1, focusX: 50, focusY: 50 });
      }
  ...
  async function replaceMedia(item: GalleryMediaItem, event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const next = items.map((entry) => entry.id === item.id ? { ...entry, mediaUrl: blob.url, mediaType: isVideo ? "video" as const : "image" as const, contentType: prepared.type, fit: isVideo ? entry.fit : "cover" as const, zoom: 1, focusX: 50, focusY: 50 } : entry);
      ...
      <section className="mx-auto max-w-6xl">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-4 max-w-2xl leading-7 text-neutral-400">Zieh das Fadenkreuz direkt im Bild an die Stelle, die im Ausschnitt bleiben soll. Danach kannst du den Zoom fein einstellen.</p>
          </div>
        ...
        <div className="mt-7 flex justify-center"><div className="flex rounded-full border border-white/10 bg-white/[0.04] p-1"><button type="button" onClick={() => setPreviewMode("mobile")} className={`rounded-full px-4 py-2 text-xs font-bold uppercase ${previewMode === "mobile" ? "bg-white text-black" : "text-neutral-400"}`}>Handy</button><button type="button" onClick={() => setPreviewMode("desktop")} className={`rounded-full px-4 py-2 text-xs font-bold uppercase ${previewMode === "desktop" ? "bg-white text-black" : "text-neutral-400"}`}>Desktop</button></div></div>
            ...
            return (
              <article key={item.id} className="overflow-hidden rounded-3xl border border-white/10 bg-white/[0.04]">
                <div className="border-b border-white/10 bg-black p-3 sm:p-4">
                  <div className="mb-3 flex items-center justify-between gap-3"><span className="text-xs font-bold uppercase tracking-[0.25em] text-orange-300">Website-Vorschau · {index + 1}</span><div className="flex gap-1.5"><button type="button" onClick={() => move(index, -1)} disabled={index === 0} className="rounded-full border border-white/15 px-3 py-1.5 text-sm disabled:opacity-25">↑</button><button type="button" onClick={() => move(index, 1)} disabled={index === items.length - 1} className="rounded-full border border-white/15 px-3 py-1.5 text-sm disabled:opacity-25">↓</button></div></div>
                  <div className={`mx-auto overflow-hidden rounded-2xl border border-white/10 bg-neutral-950 ${previewMode === "mobile" ? "max-w-[390px]" : "max-w-[620px]"}`}>
                    <div className={`relative flex items-center justify-center overflow-hidden ${previewHeight}`}>{item.mediaType === "video" ? <video src={item.mediaUrl} controls playsInline preload="metadata" className="h-full w-full object-contain" /> : <img src={item.mediaUrl} alt="Galerie Vorschau" className="h-full w-full" style={imageStyle(item)} />}</div>
                    {caption && <p className="border-t border-white/10 px-5 py-4 text-sm leading-6 text-neutral-400">{caption}</p>}
                  </div>
                </div>

                <div className="p-4 sm:p-5">
                  <div className="grid grid-cols-2 gap-2"><label className="flex cursor-pointer items-center justify-center rounded-2xl bg-white px-4 py-3 text-center text-sm font-black text-black">{replacingId === item.id ? "Ersetzt …" : "Bild ersetzen"}<input type="file" accept="image/jpeg,image/png,image/webp,image/gif,video/mp4,video/quicktime,video/webm" onChange={(event) => replaceMedia(item, event)} disabled={Boolean(replacingId) || saving} className="hidden" /></label><button type="button" onClick={() => remove(item)} disabled={saving} className="rounded-2xl border border-red-400/30 px-4 py-3 text-sm font-bold text-red-300">Löschen</button></div>

                  {item.mediaType === "image" && <div className="mt-4 rounded-2xl border border-orange-300/20 bg-black/40 p-4">
                    <div className="mb-4 flex gap-2"><button type="button" onClick={() => updateItem(item.id, { fit: "contain", zoom: 1 })} className={`flex-1 rounded-xl border px-3 py-2.5 text-xs font-bold ${fit === "contain" ? "border-orange-300 bg-orange-300 text-black" : "border-white/15 text-neutral-300"}`}>Ganzes Bild</button><button type="button" onClick={() => updateItem(item.id, { fit: "cover" })} className={`flex-1 rounded-xl border px-3 py-2.5 text-xs font-bold ${fit === "cover" ? "border-orange-300 bg-orange-300 text-black" : "border-white/15 text-neutral-300"}`}>Rahmen füllen</button></div>
                    <ImageFocusEditor src={item.mediaUrl} zoom={zoom} focusX={focusX} focusY={focusY} aspectClass={previewMode === "mobile" ? "aspect-[1/1.05]" : "aspect-[3/2]"} onChange={(next) => updateItem(item.id, { ...next, fit: (next.zoom ?? zoom) > 1 ? "cover" : fit, position: "center" })} />
                  </div>}
