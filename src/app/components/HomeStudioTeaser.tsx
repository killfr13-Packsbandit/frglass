...
export default function HomeStudioTeaser() {
  ...
  const eyebrow = get(
    `home.studio.eyebrow.${lang}`,
    language === "de" ? "Werkstatt" : "Studio",
  );
  const title = get(
    `home.studio.title.${lang}`,
  ...
  return (
    <section className="bg-neutral-950 px-4 py-14 text-white sm:px-6 sm:py-20">
      <div className="mx-auto grid max-w-6xl overflow-hidden rounded-[1.5rem] border border-white/10 bg-white/[0.03] sm:rounded-[2rem] xl:grid-cols-[0.9fr_1.1fr]">
        <div className="relative min-h-[240px] bg-neutral-950 sm:min-h-[320px] xl:min-h-[360px]">
          {mediaUrl &&
            (mediaType === "video" ? (
              <video
                src={mediaUrl}
          ...
          <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-transparent xl:bg-gradient-to-r" />
        </div>

        <div className="flex flex-col justify-center p-6 sm:p-10 xl:p-12">
          ...
          <Link
            href="/studio"
            className="mt-7 inline-flex w-fit rounded-full bg-white px-5 py-3 text-xs font-black uppercase tracking-wider text-black transition hover:bg-orange-200 sm:px-6 sm:text-sm"
          >
            {button}
          </Link>
        </div>
