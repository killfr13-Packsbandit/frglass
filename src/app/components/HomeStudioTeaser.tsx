...
export default function HomeStudioTeaser() {
  ...
  const zoom = Number(get("home.studio.media.zoom", "1")) || 1;
  const x = Number(get("home.studio.media.focusX", "50")) || 50;
  const y = Number(get("home.studio.media.focusY", "50")) || 50;
  const style = { objectPosition: `${x}% ${y}%`, transform: `scale(${zoom})`, transformOrigin: `${x}% ${y}%` };
  ...
  return <section className="bg-neutral-950 px-4 py-14 text-white sm:px-6 sm:py-20"><div className="mx-auto grid max-w-6xl overflow-hidden rounded-[1.5rem] border border-white/10 bg-white/[0.03] sm:rounded-[2rem] xl:grid-cols-[0.9fr_1.1fr]">
    <div className="relative min-h-[240px] overflow-hidden bg-neutral-950 sm:min-h-[320px] xl:min-h-[360px]">{mediaUrl && (mediaType === "video" ? <video src={mediaUrl} controls playsInline preload="metadata" className="absolute inset-0 h-full w-full object-cover" style={style} /> : <img src={mediaUrl} alt={language === "de" ? "FRGLASS Werkstatt" : "FRGLASS studio"} className="absolute inset-0 h-full w-full object-cover" style={style} />)}<div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-transparent xl:bg-gradient-to-r" /></div>
