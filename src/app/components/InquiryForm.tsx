...
const copy = {
  de: {
    ...
    productMessage: (productName: string) =>
      `Hallo Florian, ich interessiere mich für „${productName}“. Ist das Stück noch verfügbar?`,
  ...
  en: {
    ...
    productMessage: (productName: string) =>
      `Hi Florian, I’m interested in “${productName}”. Is this piece still available?`,
...
export default function InquiryForm({ productName, productSlug }: Props) {
  ...
  const fallbackHref = useMemo(() => {
    const subject = productName
      ...
      : "FRGLASS inquiry";
    const body = [
      productName ? `${language === "de" ? "Stück" : "Piece"}: ${productName}` : "",
      productSlug ? `https://frglass.at/shop/${productSlug}` : "",
      "",
      ...
      <div className="mt-7 grid gap-5">
        ...
        <label className="grid gap-2">
          ...
          <textarea
            ...
            className="resize-y rounded-2xl border border-white/10 bg-black/50 px-4 py-3 outline-none transition focus:border-orange-300/60"
          />
        ...
        <button
          type="submit"
          disabled={sending}
          className="rounded-full bg-orange-300 px-6 py-4 text-sm font-black uppercase tracking-wider text-black transition hover:bg-white disabled:cursor-wait disabled:opacity-50"
        >
