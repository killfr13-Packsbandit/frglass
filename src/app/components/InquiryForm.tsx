    ...
    >
      ...
      {productName && (
        <p className="mt-3 text-sm text-neutral-400">
          {t.product}: <span className="font-bold text-white">{productName}</span>
        </p>
      )}
      ...
      <div className="mt-7 grid gap-5">
        ...
        <label className="flex items-start gap-3 text-sm leading-6 text-neutral-400">
          ...
          <span>
            {t.privacyStart}{" "}
            <Link href="/datenschutz" className="text-orange-300 underline-offset-4 hover:underline">{t.privacyLink}</Link>{" "}
            {t.privacyEnd}
          </span>
        </label>
        ...
        <button type="submit" disabled={sending} className="rounded-full bg-orange-300 px-6 py-3.5 text-sm font-black uppercase tracking-wider text-black transition hover:bg-white disabled:cursor-wait disabled:opacity-50">
